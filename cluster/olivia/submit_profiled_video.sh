#!/usr/bin/env bash
# Submit one profiled RREA capture job on OLIVIA.
#
#   python scripts/olivia.py script cluster/olivia/submit_profiled_video.sh \
#       RUN_TAG=<tag> \
...
import json, os, sys
layout = json.load(open(sys.argv[1]))['olivia_layout']
for key, variable in {
    'mpi_ranks': 'NTASKS',
...
SBATCH_FILE="${REPO}/cluster/olivia/slurm_rrea_profiled_video_capture.sbatch"
SBATCH_ARGS=(
    --ntasks="${NTASKS}"
