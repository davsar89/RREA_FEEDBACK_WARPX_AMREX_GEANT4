...
- [`RREA_WARPX_AMREX_ARCHITECTURE_GUIDE.pdf`](RREA_WARPX_AMREX_ARCHITECTURE_GUIDE.pdf)
  (`.tex` source): current physics/software architecture, including its
  source-defined data-flow diagram.
- [`rrea_pic_explainer.pdf`](rrea_pic_explainer.pdf) (`.tex` source): narrative
  explainer of how RREA with feedback is
...
- `RREA_RERUN_GUIDE.md`: the current local validation, restart procedure,
  HPC capture submission, and all-physics Coleman-Dwyer campaign
  workflow.
- `HPC_CONNECTION_AND_2FA.md`: the single remote HPC connection and
  authentication procedure.
- `HPC_COMMAND_WORKFLOW_NOTES.md`: source transfer, Slurm build/run,
  monitoring, quota, retrieval, and the ordered capture-to-deliverable
  procedure (reduce on the host, render, pull, draw).

LaTeX build intermediates are local artifacts. The two reader-facing PDFs above
are tracked builds of their authoritative `.tex` sources.
...
The two compiled guide PDFs are tracked reader-facing build products; other
generated PDFs stay untracked. Cite a repo paper when present, otherwise its
DOI.
