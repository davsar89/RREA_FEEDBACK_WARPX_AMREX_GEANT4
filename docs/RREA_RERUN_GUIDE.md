...
Local and OLIVIA runs use the same canonical values in
...
forks. Local source preparation uses the WarpX submodule by default; the OLIVIA
build helper uses the pinned WarpX/AMReX refs and commits.
...
`olivia.py sync`; there is no separate table transfer. Its semantics belong to
...
2. For OLIVIA, check `olivia.py jobs` and `olivia.py quota`, then run
   `olivia.py sync`.
...
The table travels with `olivia.py pull-restart`/`push-restart`.
...
checkpoint between OLIVIA and this laptop at a different rank count. The
...
Use `cluster/olivia/submit_rrea_cd_campaign.py`. The campaign keeps its fixed
...
OLIVIA commands live only in `docs/OLIVIA_COMMAND_WORKFLOW_NOTES.md`.
