    ...
    int step)
{
    ...
    int const ranks = amrex::ParallelDescriptor::NProcs();
    auto canonical_position = [&](double value, int dim) {
        if (!geom.isPeriodic(dim)) return value;
    ...
    auto phase_bin = [](double ux, double uy, double uz, double theta) {
        ...
        exponent = std::max(-1024, std::min(1023, exponent));
        return (exponent + 1024) * 8
            + (ur >= 0.0 ? 1 : 0)
            + (utheta >= 0.0 ? 2 : 0)
            + (uz >= 0.0 ? 4 : 0);
    };
    auto record = [&](double r, double z, double ux, double uy, double uz,
                      double theta, double weight, bool candidate,
                      std::uint64_t high, std::uint64_t low) {
        r = canonical_position(r, 0);
        z = canonical_position(z, 1);
        ...
        if (!std::isfinite(r_cell) || !std::isfinite(z_cell)
            ...
            amrex::Abort("adaptive_resample_v1 found an invalid global CIC support");
        }
        rrea::RreaGlobalCicParticle result;
        result.support_r = static_cast<int>(i0);
        result.support_z = static_cast<int>(j0);
        result.phase = phase_bin(ux, uy, uz, theta);
    ...
    auto build_plan = [&](std::string const& species_name,
        ...
        RreaForEachValidParticle(
            ...
            [&](RreaLiveParticle const& p) {
                if (std::find(
                        ...
                        p.StableKey()) != pending_removals.end()) {
                    return;
                }
                local.push_back(record(
                    p.r, p.z, p.ux, p.uy, p.uz, p.theta, p.weight, false, 0,
                    static_cast<std::uint64_t>(p.idcpu)));
                result.live.push_back(LiveSlot{
                    static_cast<std::uint64_t>(p.idcpu), &p.idcpu, &p.weight,
                    p.KineticEnergyEv(), p.weight});
            });
        ...
        for (std::size_t begin = 0; begin < owned.size();) {
            ...
            std::vector<rrea::RreaCicResampleParticle> support;
            support.reserve(end - begin);
            for (std::size_t i = begin; i < end; ++i) {
                support.push_back({owned[i].fraction_r, owned[i].fraction_z,
                                   owned[i].weight});
            }
            std::size_t const capacity =
                rrea::RreaCicResampleRemovalCapacity(support);
        ...
        for (auto const& group : groups) {
            ...
            auto const request = static_cast<std::size_t>(found->second.request);
            if (request == 0) continue;
            std::vector<rrea::RreaGlobalCicParticle> support(
                owned.begin() + static_cast<std::ptrdiff_t>(group.begin),
                owned.begin() + static_cast<std::ptrdiff_t>(group.end));
            auto plan = rrea::RreaPlanGlobalCicThinning<amrex::ParticleReal>(
                support, support.size() - request, ranks, stream_key,
                [&](std::uint64_t group_id, std::size_t reduction) {
                    return RreaAdaptiveResampleCicUniform(
                        m_rng_seed, group_id ^ m_population_controller_rng_salt,
                        static_cast<std::uint64_t>(step), reduction, rng_species);
                });
            if (!plan.valid() || plan.capacity != group.summary.capacity
                || plan.geometric_floor != group.summary.floor
                || plan.removed == 0
                || plan.removed > static_cast<std::size_t>(
            ...
            for (auto const& update : plan.updates) {
        ...
        for (auto& slot : result.live) {
            ...
            account(*slot.weight, slot.planned_weight, slot.energy_eV);
    ...
    long target)
{
    ...
    if (RreaAdaptiveResampleAction(count, target, split_min_w)
        ...
        RreaForEachValidParticle(species, [&](RreaLiveParticle const& p) {
            // One primitive, pinned by the native smoke -- do not reimplement
            // the rule inline.
            auto const outcome = RreaAdaptiveResampleSplitOutcome(
                static_cast<double>(p.weight), split_min_w);
            if (!outcome.split) {
                return;
            }
            auto const child_w =
                static_cast<amrex::ParticleReal>(outcome.child_weight);
            p.weight = child_w;
            cx.push_back(p.r * std::cos(p.theta));
            cy.push_back(p.r * std::sin(p.theta));
            cz.push_back(p.z);
            cpx.push_back(p.ux);
            cpy.push_back(p.uy);
            cpz.push_back(p.uz);
            cw.push_back(child_w);
        });
