...
// Tapered background potential. With the taper off, use the z-only profile.
...
{
    if (!config.TaperActive()) {
        return background_phi_from_z(z, config);
    }
    double const f = taper_f(r, config);
    return phi_ref + f * (background_phi_from_z(z, config) - phi_ref);
}
        ...
        amrex::LoopOnCpu(
            ...
            [&](int i, int j, int k) {
                ...
                if (periodic_z) {
                    ...
                    z = plo[z_dir]
                        + (static_cast<double>(wrapped - zlo) + 0.5) * dx[z_dir];
                } else if (iz < zlo) {
                    z = z_face_lo;
                } else if (iz > zhi) {
                    z = z_face_hi;
                    ...
                    z = plo[z_dir]
                        + (static_cast<double>(iz - zlo) + 0.5) * dx[z_dir];
                }
                double r = 0.0;
#if (AMREX_SPACEDIM >= 2)
                // Ghost values sit ON the physical faces (levelbc contract):
                // clamp r the same way z is clamped above.  The low-r ghost
                // is behind the zero-flux axis face and is never read.
                if (i < rlo) {
                    r = r_face_lo;
                } else if (i > rhi) {
                    r = r_face_hi;
                    ...
                    r = plo[0]
                        + (static_cast<double>(i - rlo) + 0.5) * dx[0];
                }
#endif
                arr(i, j, k) = background_phi(r, z, phi_ref, config);
            });
    }
    if (periodic_z) {
