...
{
    try {
        // The profile stores its rows in the managed arena so device code can
        // read them through a KineticSpan, so it cannot be loaded before AMReX
        // brings that arena up: the allocator returns null and the first row
        // construction faults.
        rrea::smoke::GpuSession const session;
