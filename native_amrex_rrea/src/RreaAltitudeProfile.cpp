    ...
    bool require_positive)
{
    ...
    m_altitude_m = std::move(altitudes);
...
amrex::Real RreaAltitudeProfile::Interpolate(amrex::Real altitude_m) const
{
    ...
    auto const upper = std::upper_bound(m_altitude_m.begin(), m_altitude_m.end(), altitude_m);
    if (upper == m_altitude_m.begin()) {
        return m_value.front();
    }
    if (upper == m_altitude_m.end()) {
        return m_value.back();
    }
    std::size_t const hi = static_cast<std::size_t>(upper - m_altitude_m.begin());
    std::size_t const lo = hi - 1U;
    amrex::Real const span = m_altitude_m[hi] - m_altitude_m[lo];
    amrex::Real const t = span > amrex::Real(0.0)
        ? (altitude_m - m_altitude_m[lo]) / span
        : amrex::Real(0.0);
    return (amrex::Real(1.0) - t) * m_value[lo] + t * m_value[hi];
}
