...
rrea::MaxwellTMRZ gradient_field(int nr, int nz, double dr, double dz)
{
    ...
    rrea::GpuVector<double> phi(static_cast<std::size_t>(f.nr) * f.nz, 0.0);
...
void light_cone_and_absorber()
{
    ...
    rrea::GpuVector<double> jz(f.ez.size(), 0.0);
...
void conductive_relaxation()
{
    ...
    rrea::GpuVector<double> sigma_r(f.er.size(), sigma0);
    rrea::GpuVector<double> sigma_z(f.ez.size(), sigma0);
...
void gauss_constraint_follows_continuity_current()
{
    ...
    rrea::GpuVector<double> jr(f.er.size(), 0.0);
    rrea::GpuVector<double> jz(f.ez.size(), 0.0);
    for (int j = 0; j < f.nz; ++j) {
        for (int i = 1; i <= f.nr; ++i) {
...
void closed_domain_energy_is_conserved()
{
    ...
    double worst = 0.0;
    for (int n = 0; n < 200; ++n) {
        rrea::GpuVector<double> const b_previous = f.btheta;
        rrea::MaxwellAdvanceB(f, dt);
...
void joule_and_poynting_close_the_open_step()
{
    ...
    double const dt = 2.5e-9;
    rrea::GpuVector<double> const b_before = f.btheta;
    rrea::MaxwellAdvanceB(f, dt);
