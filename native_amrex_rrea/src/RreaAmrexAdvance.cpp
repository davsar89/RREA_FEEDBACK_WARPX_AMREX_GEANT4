        ...
        for (amrex::MFIter mfi(face); mfi.isValid(); ++mfi) {
            ...
            amrex::LoopOnCpu(box, [&](int i, int j, int k) {
                if (dir == 0) {
                    ...
                    double spacing = dx[1];
                    if (!geom.isPeriodic(1) && (j <= jlo || j > jhi)) {
                        spacing = 0.5 * dx[1];
                    }
                    e(i, j, k) = -(
                        static_cast<double>(p(i, j, k))
...
std::vector<double> face_multiplicity(
    amrex::MultiFab const& faces, int stride, std::size_t size)
{
    std::vector<double> count(size, 0.0);
    for (amrex::MFIter mfi(faces); mfi.isValid(); ++mfi) {
        amrex::LoopOnCpu(mfi.validbox(), [&](int i, int j, int) {
...
void face_to_full_vector(
    ...
    std::vector<double> const& component_weights,
    int stride,
    std::vector<double> const& count,
    double* value_out,
    std::size_t value_size)
{
    std::fill(value_out, value_out + value_size, 0.0);
    for (amrex::MFIter mfi(faces); mfi.isValid(); ++mfi) {
        auto const arr = faces.const_array(mfi);
        amrex::LoopOnCpu(mfi.validbox(), [&](int i, int j, int k) {
            std::size_t const f = static_cast<std::size_t>(i)
                + static_cast<std::size_t>(stride) * j;
            for (std::size_t c = 0; c < component_weights.size(); ++c) {
                value_out[f] += component_weights[c]
                    * static_cast<double>(arr(i, j, k, static_cast<int>(c)));
            }
        });
    }
    for (std::size_t f = 0; f < value_size; ++f) {
        if (count[f] > 0.0) { value_out[f] /= count[f]; }
    }
}
...
void RreaAmrexAdvance::CommitMaxwellTotalField()
{
    ...
    for (std::size_t f = 0; f < total_r.size(); ++f) {
        total_r[f] = m_maxwell.er[f] + m_ambient.er[f];
    }
    for (std::size_t f = 0; f < total_z.size(); ++f) {
        total_z[f] = m_maxwell.ez[f] + m_ambient.ez[f];
    }
    full_vector_to_face(total_r, m_maxwell.nr + 1, *m_face_efield[0][0]);
    full_vector_to_face(total_z, m_maxwell.nr, *m_face_efield[0][1]);
    ...
    for (amrex::MFIter mfi(*level.efield_x); mfi.isValid(); ++mfi) {
        ...
        amrex::LoopOnCpu(mfi.validbox(), [&](int i, int j, int k) {
            ex(i, j, k) = static_cast<amrex::Real>(0.5 * (
                total_r[m_maxwell.ErIndex(i, j)]
                + total_r[m_maxwell.ErIndex(i + 1, j)]));
            ez_cell(i, j, k) = static_cast<amrex::Real>(0.5 * (
                total_z[m_maxwell.EzIndex(i, j)]
                + total_z[m_maxwell.EzIndex(i, j + 1)]));
        });
    }
...
double RreaAmrexAdvance::MaxwellBthetaAt(double r_m, double z_m) const noexcept
{
    ...
    double const fr = std::clamp(r_m / m_maxwell.dr, 0.0,
        static_cast<double>(m_maxwell.nr));
    double const fz = std::clamp((z_m - m_maxwell.z_lo) / m_maxwell.dz, 0.0,
        static_cast<double>(m_maxwell.nz));
    int const i0 = std::min(static_cast<int>(std::floor(fr)), m_maxwell.nr - 1);
    int const j0 = std::min(static_cast<int>(std::floor(fz)), m_maxwell.nz - 1);
    int const i1 = i0 + 1;
    int const j1 = j0 + 1;
    double const tr = fr - i0;
    double const tz = fz - j0;
    return (1.0 - tr) * ((1.0 - tz) * m_maxwell.Bt(i0, j0)
                         + tz * m_maxwell.Bt(i0, j1))
        + tr * ((1.0 - tz) * m_maxwell.Bt(i1, j0)
                + tz * m_maxwell.Bt(i1, j1));
}

void RreaAmrexAdvance::GatherTotalFieldRZ(
    ...
    double& er, double& ez, double& btheta) const noexcept
{
    ...
    auto const build = RreaFieldGatherStencilFor(
        RreaCellCenteredGatherIndexCoordinate(
            r_m, geom.ProbLo(0), m_maxwell.dr, domain.smallEnd(0)),
        RreaCellCenteredGatherIndexCoordinate(
            z_m, geom.ProbLo(1), m_maxwell.dz, domain.smallEnd(1)),
    ...
    auto const wrap_j = [&](int j) {
        return periodic_z ? (j + m_maxwell.nz) % m_maxwell.nz : j;
    };
    ...
    auto const cell_er = [&](int i, int j) {
        double const lo = m_maxwell.er[m_maxwell.ErIndex(i, j)]
            + m_ambient.er[m_maxwell.ErIndex(i, j)];
        double const hi = m_maxwell.er[m_maxwell.ErIndex(i + 1, j)]
            + m_ambient.er[m_maxwell.ErIndex(i + 1, j)];
        return 0.5 * (lo + hi);
    };
    auto const cell_ez = [&](int i, int j) {
        double const lo = m_maxwell.ez[m_maxwell.EzIndex(i, j)]
            + m_ambient.ez[m_maxwell.EzIndex(i, j)];
        double const hi = m_maxwell.ez[m_maxwell.EzIndex(i, j + 1)]
            + m_ambient.ez[m_maxwell.EzIndex(i, j + 1)];
        return 0.5 * (lo + hi);
    };
    er = RreaBilinearFromCorners(
             cell_er(i0, j0), cell_er(i0, j1),
             cell_er(i1, j0), cell_er(i1, j1), build.tx, build.tz)
        * RreaOddRadialAxisScale(r_m, geom.ProbLo(0), m_maxwell.dr);
    ...
    RreaAmrexAdvanceConfig const& config)
{
    ...
    m_jr_average.assign(m_maxwell.er.size(), 0.0);
    m_jz_average.assign(m_maxwell.ez.size(), 0.0);
    ...
    for (int substep = 0; substep < coupling_substeps; ++substep) {
        ...
        for (std::size_t f = 0; f < m_jr_total.size(); ++f) {
            m_jr_average[f] += m_jr_total[f] / coupling_substeps;
        }
        for (std::size_t f = 0; f < m_jz_total.size(); ++f) {
            m_jz_average[f] += m_jz_total[f] / coupling_substeps;
        }
    ...
    m_last_diagnostics.maxwell_btheta_max_t = 0.0;
    for (double const value : m_maxwell.btheta) {
        m_last_diagnostics.maxwell_btheta_max_t = std::max(
            m_last_diagnostics.maxwell_btheta_max_t, std::abs(value));
    }
...
void RreaAmrexAdvance::AssembleTotalFaceCurrent()
{
    ...
    std::vector<double> const species_charge = {qe, -qe, -qe};
    ...
    face_to_full_vector(
        *m_face_number_flux[0][0], species_charge, m_maxwell.nr + 1,
        m_er_face_count, m_current_buffer.data(), er_faces);
    face_to_full_vector(
        *m_face_number_flux[0][1], species_charge, m_maxwell.nr,
    ...
    for (std::size_t f = 0; f < er_faces; ++f) {
        m_current_buffer[f] += m_jr_kinetic[f];
    }
    for (std::size_t f = 0; f < ez_faces; ++f) {
        m_current_buffer[er_faces + f] += m_jz_kinetic[f];
    }
    amrex::ParallelDescriptor::ReduceRealSum(
        m_current_buffer.data(), static_cast<int>(m_current_buffer.size()));
    m_jr_total.assign(
        m_current_buffer.begin(), m_current_buffer.begin() + er_faces);
    m_jz_total.assign(
        m_current_buffer.begin() + er_faces, m_current_buffer.end());
}
    ...
    std::vector<double> const& jz_total)
{
    ...
    double local_signed_e = 0.0;
    int local_max_i = -1;
    int local_max_j = -1;
    double local_max_old = 0.0;
    double local_max_new = 0.0;
    double local_max_divergence = 0.0;
    double local_max_residual = 0.0;
    double local_max_residual_e = 0.0;
    auto const& rho_new = *m_rho_total[0];
    auto const& rho_old = *m_maxwell_rho_previous;
    for (amrex::MFIter mfi(rho_new); mfi.isValid(); ++mfi) {
        auto const box = mfi.validbox();
        auto const new_arr = rho_new.const_array(mfi);
        auto const old_arr = rho_old.const_array(mfi);
        auto const particle_arr = level.rho_c_per_m3->const_array(mfi);
        auto const fluid_arr = fluid.FluidChargeDensity().const_array(mfi);
        // Charge-neutral source channels can be individually large; their
        // gross outer-step charge belongs in this backward-error scale.
        auto const source_e = fluid.LowElectronSourceRate().const_array(mfi);
        auto const source_pos = fluid.PositiveIonSourceRate().const_array(mfi);
        auto const source_neg = fluid.DirectNegativeIonSourceRate().const_array(mfi);
        amrex::LoopOnCpu(box, [&](int i, int j, int k) {
            // The four divergence operands separately: the gate's scale is
            // their gross magnitude, not only their signed sum.
            double const r_c_dr = (i + 0.5) * m_maxwell.dr * m_maxwell.dr;
            double const radial_hi =
                (i + 1) * m_maxwell.dr * jr_total[m_maxwell.ErIndex(i + 1, j)]
                / r_c_dr;
            double const radial_lo =
                i * m_maxwell.dr * jr_total[m_maxwell.ErIndex(i, j)] / r_c_dr;
            double const axial_hi =
                jz_total[m_maxwell.EzIndex(i, j + 1)] / m_maxwell.dz;
            double const axial_lo =
                jz_total[m_maxwell.EzIndex(i, j)] / m_maxwell.dz;
            double const old_value = static_cast<double>(old_arr(i, j, k));
            double const new_value = static_cast<double>(new_arr(i, j, k));
            double const residual = new_value - old_value
                + dt_s * (radial_hi - radial_lo + axial_hi - axial_lo);
            double const particle_value = static_cast<double>(
                particle_arr(i, j, k));
            double const fluid_value = static_cast<double>(
                fluid_arr(i, j, k));
            double const relative_scale = std::abs(particle_value)
                + std::abs(fluid_value) + std::abs(old_value)
                + dt_s * (std::abs(radial_hi) + std::abs(radial_lo)
                          + std::abs(axial_hi) + std::abs(axial_lo))
                + dt_s * qe * (
                    std::max(static_cast<double>(source_e(i, j, k)), 0.0)
                    + std::max(static_cast<double>(source_pos(i, j, k)), 0.0)
                    + std::max(static_cast<double>(source_neg(i, j, k)), 0.0));
            double const cell_volume = m_maxwell.CellVolume(i);
            double const residual_e = std::abs(residual)
                * cell_volume / qe;
            double const absolute_floor = absolute_allowance_e * qe
                / (cell_volume * kMaterialContinuityRelativeTolerance);
            double relative = std::abs(residual) / std::max(
                {relative_scale, absolute_floor,
                 std::numeric_limits<double>::min()});
            if (!std::isfinite(relative)) {
                relative = std::numeric_limits<double>::infinity();
            }
            local_max_abs_e = std::max(local_max_abs_e, residual_e);
            local_signed_e += residual * cell_volume / qe;
            if (relative > local_max_relative) {
                local_max_relative = relative;
                local_max_i = i;
                local_max_j = j;
                local_max_old = old_value;
                local_max_new = new_value;
                local_max_divergence =
                    radial_hi - radial_lo + axial_hi - axial_lo;
                local_max_residual = residual;
                local_max_residual_e = residual_e;
            }
        });
    }
    ...
    if (!std::isfinite(local_max_relative)
        ...
        int location[2] = {local_max_i, local_max_j};
        double details[5] = {
            local_max_old,
            local_max_new,
            local_max_divergence,
            local_max_residual,
            local_max_residual_e};
...
void RreaAmrexAdvance::RecordGaussMonitor()
{
    ...
    for (amrex::MFIter mfi(rho); mfi.isValid(); ++mfi) {
        ...
        amrex::LoopOnCpu(mfi.validbox(), [&](int i, int j, int k) {
            double const rho_value = static_cast<double>(rho_arr(i, j, k));
            residual_max = std::max(residual_max, std::abs(
                eps0 * m_maxwell.DivergenceRZ(m_maxwell.er, m_maxwell.ez, i, j)
                - rho_value));
            scale_max = std::max(scale_max, std::abs(rho_value));
        });
    }
