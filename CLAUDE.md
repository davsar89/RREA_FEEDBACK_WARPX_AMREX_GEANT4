...
- Before any OLIVIA build or simulation submission, first complete a clean
...
CHECKPOINTS MOVE BETWEEN OLIVIA AND THIS LAPTOP, AND MUST KEEP DOING SO —
that is how a run continues when the compute allocation runs out, and the
...
`olivia.py pull-checkpoint`, keep
...
Fetch a whole run for local continuation with `olivia.py pull-restart` — the
...
Architecture, rerun, and OLIVIA procedures are indexed by `docs/README.md`;
