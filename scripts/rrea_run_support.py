...
def load_rrea_defaults(path: Path = DEFAULTS_PATH) -> dict[str, Any]:
    ...
    layout = defaults.get("hpc_layout")
    if not isinstance(layout, dict):
        raise SystemExit(f"{path}: hpc_layout must be an object")
    for key in ("mpi_ranks", "cpus_per_rank", "transport_omp_threads"):
        value = layout.get(key)
        if not isinstance(value, int) or value <= 0:
            raise SystemExit(f"{path}: hpc_layout.{key} must be a positive integer")
    if layout["transport_omp_threads"] > layout["cpus_per_rank"]:
        raise SystemExit(
            f"{path}: hpc_layout.transport_omp_threads cannot exceed cpus_per_rank")
...
_ALLOWED_USER_OVERRIDE_EXACT = {"warpx.verbose"}
...
def rotate_checkpoints(
    ...
    The cluster sbatch rotates separately. `min_age_s` keeps an active writer
