#!/usr/bin/env python3
"""Answer OLIVIA SSH password and OATH prompts without logging credentials.

The password comes from a local environment variable or private local file. The
TOTP normally comes from ``OLIVIA_TOTP_CODE``; when the explicitly authorized
``OLIVIA_TOTP_AUTOMATED=1`` mode is selected, it is derived in memory from a
Base32 ``OLIVIA_TOTP_SECRET`` or the private ``authenticator_qr.jpg``.
...
def _local_file(
    ...
    state_root = os.environ.get("OLIVIA_STATE_DIR", "").strip()
    if not state_root:
        state_root = str(
            Path.home() / ".local" / "share" / "rrea_warpx_amrex" / "olivia"
        )
    state_path = Path(state_root) / state_filename
    if state_path.is_file():
        return state_path
    project = os.environ.get("OLIVIA_LOCAL_PROJECT", "").strip()
...
def _temporary_credential(wanted_key: str) -> str | None:
    path_text = os.environ.get("OLIVIA_ASKPASS_CREDENTIAL_FILE", "").strip()
...
def read_password() -> str:
    env_file = _local_file(
        "OLIVIA_ENV_FILE", "credentials.env", "olivia.env"
    )
    local_note = _local_file(
        "OLIVIA_PASSWORD_FILE", "password_note.txt", "REMOTE_OLIVIA.md"
    )
    for source in (
        os.environ.get("OLIVIA_PASSWORD", "").strip(),
        _temporary_credential("OLIVIA_PASSWORD"),
        _value_from_env_file(env_file, "OLIVIA_PASSWORD"),
        _password_from_local_note(local_note),
    ):
        if source:
            return source
    raise SystemExit(
        "No OLIVIA password found. Set OLIVIA_PASSWORD or configure "
        "OLIVIA_STATE_DIR with a mode-0600 credentials.env file."
    )
...
def _qr_text() -> str:
    ...
    if qr_path is None or not qr_path.is_file():
        raise SystemExit(
            "No local OLIVIA QR image found; use interactive TOTP or set "
            "OLIVIA_QR_FILE to the private authenticator export."
        )
    if shutil.which("zbarimg"):
        try:
            ...
            raise SystemExit("The local OLIVIA QR image could not be decoded.") from exc
    else:
        try:
            import cv2
        ...
        if image is None:
            raise SystemExit("The local OLIVIA QR image could not be read.")
        decoded, _points, _straight = cv2.QRCodeDetector().detectAndDecode(image)
        decoded = decoded.strip()
    if not decoded:
        raise SystemExit("The local OLIVIA QR image could not be decoded.")
    return decoded
...
def _base32_secret(text: str) -> bytes:
    ...
    try:
        ...
        raise SystemExit("The OLIVIA TOTP credential has an invalid Base32 secret.") from exc
...
def _first_query_value(query: dict[str, list[str]], key: str) -> str:
    ...
    if not values or not values[0]:
        raise SystemExit(f"The OLIVIA QR is missing required TOTP field {key}.")
    return values[0]
...
def _totp_parameters_from_qr() -> tuple[bytes, int, Any, int]:
    ...
    if start < 0:
        raise SystemExit("The OLIVIA QR does not contain an authenticator record.")
    ...
    if parsed.scheme == "otpauth" and parsed.netloc == "totp":
        ...
        try:
            ...
            raise SystemExit("The OLIVIA QR has invalid TOTP numeric parameters.") from exc
        digest = _digest(query.get("algorithm", ["SHA1"])[0])
        if digits not in (6, 8) or period <= 0:
            raise SystemExit("The OLIVIA QR has unsupported TOTP digits or period.")
        return secret, digits, digest, period

    if parsed.scheme != "otpauth-migration":
        raise SystemExit("The OLIVIA QR uses an unsupported authenticator format.")
    ...
    try:
        ...
        raise SystemExit("The OLIVIA authenticator migration record is invalid.") from exc
    candidates = [
        item for item in candidates if item["type"] == 2 and item["secret"]
    ]
    if not candidates:
        raise SystemExit("The OLIVIA QR contains no TOTP account.")

    hint = os.environ.get("OLIVIA_TOTP_LABEL_CONTAINS", "NRIS").strip().lower()
    matches = [
        item
    ...
    if len(matches) == 1:
        ...
        raise SystemExit(
            "The OLIVIA QR contains multiple TOTP accounts and the configured "
            "OLIVIA_TOTP_LABEL_CONTAINS hint did not select exactly one."
        )
    ...
    if algorithm is None or digits is None:
        raise SystemExit("The OLIVIA QR uses unsupported TOTP parameters.")
...
def _totp_from_local_credential() -> str:
    ...
    setup_key = (
        os.environ.get("OLIVIA_TOTP_SECRET", "").strip()
        or _temporary_credential("OLIVIA_TOTP_SECRET")
        or _value_from_env_file(env_file, "OLIVIA_TOTP_SECRET")
    )
    if setup_key:
...
def read_totp() -> str:
    code = (
        os.environ.get("OLIVIA_TOTP_CODE", "").strip()
        or (_temporary_credential("OLIVIA_TOTP_CODE") or "").strip()
    )
    if code:
        if not re.fullmatch(r"[0-9]{6}|[0-9]{8}", code):
            raise SystemExit("OLIVIA_TOTP_CODE must contain six or eight digits.")
        return code
    if os.environ.get("OLIVIA_TOTP_AUTOMATED", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    raise SystemExit(
        "No transient OLIVIA_TOTP_CODE is available. Run the socket opener "
        "interactively or explicitly authorize OLIVIA_TOTP_AUTOMATED=1."
    )
